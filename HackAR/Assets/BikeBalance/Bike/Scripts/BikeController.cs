...
{
    /// <summary>
    /// BikeController allows you to control a bike, which consists of two WheelColliders and a RigidBody.
    /// </summary>
    public class BikeController : MonoBehaviour
    {
        ...
        /// </summary>
        [Tooltip("Front wheel image (tyre and rim)")]
        public Transform frontImage;
        /// <summary>
        /// Rear wheel image (tyre and rim)
        /// </summary>
        [Tooltip("Rear wheel image (tyre and rim)")]
        ...
        /// </summary>
        [Tooltip("Splashes from under the rear wheel.")]
        ...
        /// </summary>
        [Range(0, 1)]
        [Tooltip("Rear wheel slip threshold for splashing.")]
        ...
        void Update()
        {
            wheelRear.useParticle = useParticle;
            wheelFront.update();
            wheelRear.update();
        ...
        public void Init()
        {
            if (wheelFront != null)
                return;
            wheelFront = new Wheel(frontCollider, frontImage, minTralSlip, minParticleSlip);
            wheelRear = new Wheel(rearCollider, rearImage, minTralSlip, minParticleSlip);
        ...
        public void safeBrake(float acceleration)
        {
            ...
            float maxF1 = weight1 * frontCollider.forwardFriction.extremumValue * 0.5f;
            float maxf2 = weight2 * rearCollider.forwardFriction.extremumValue * 0.65f;
            float f1 = Mathf.Clamp(f / 2, -maxF1, maxF1);
            float f2 = Mathf.Clamp(f / 2, -maxf2, maxf2);
            /*
            float kRpm = 10;
        ...
        public class Wheel
        {
            ...
            public bool useParticle;
            private float minTralSlip;
            private float minParticleSlip;

            private ParticleSystem.EmissionModule emissionModule;
            private ParticleSystem.MainModule mainModule;
            private float rpmToV;
            public Wheel(WheelCollider collider, Transform image, float minTralSlip, float minParticleSlip)
            {
                this.collider = collider;
                this.image = image;
                this.minTralSlip = minTralSlip;
                this.minParticleSlip = minParticleSlip;

                trail = collider.GetComponentInChildren<TrailRenderer>();
                if (trail != null)
                    trail.alignment = LineAlignment.TransformZ;
                particle = collider.GetComponentInChildren<ParticleSystem>();
                if (particle != null)
                {
                ...
                rpmToV = Mathf.PI / 30 * collider.radius;
            }
            public void update()
            {
                ...
                if (trail != null)
                {
                    ...
                    {
                        if (Mathf.Abs(hit.forwardSlip) >= collider.forwardFriction.extremumSlip * minTralSlip ||
                            Mathf.Abs(hit.sidewaysSlip) >= collider.sidewaysFriction.extremumSlip * minTralSlip)
                            trail.emitting = true;
                        else
                        ...
                        {
                            if (hit.forwardSlip >= collider.forwardFriction.extremumSlip * minParticleSlip)
                            {
                                emissionModule.enabled = true;
                                mainModule.startSpeed = collider.rpm * rpmToV;
                            }
                            else
                                emissionModule.enabled = false;
                        }
